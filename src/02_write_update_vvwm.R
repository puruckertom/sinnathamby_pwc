...
#############################################################
koc=round(runif(Nsims, min=-10, max=10),0)
koc_list <- unlist(strsplit(l[5],","))
koc_list[1]<-(koc[Ite]+(as.numeric(koc_list)))
l[5]=paste(koc_list,collapse=",")
##########################################################
aer_aq=round(runif(Nsims, min=-15, max=15),0)
aer_aq_list <- unlist(strsplit(l[6],","))
aer_aq_list[1]<-(aer_aq[Ite]+(as.numeric(aer_aq_list)))
l[6]=paste(aer_aq_list,collapse=",")
##################################################################################
temp_ref_aer=round(runif(Nsims, min=-5, max=5),0)
temp_ref_aer_list <- unlist(strsplit(l[7],","))
temp_ref_aer_list[1]<-(temp_ref_aer[Ite]+(as.numeric(temp_ref_aer_list)))
l[7]=paste(temp_ref_aer_list,collapse=",")
######################################################################################
anae_aq=round(runif(Nsims, min=-5, max=5),0)
anae_aq_list <- unlist(strsplit(l[8],","))
anae_aq_list[1]<-(anae_aq[Ite]+(as.numeric(anae_aq_list)))
l[8]=paste(anae_aq_list,collapse=",")
#####################################################################################
temp_ref_anae=round(runif(Nsims, min=-5, max=5),0)
temp_ref_anae_list <- unlist(strsplit(l[9],","))
temp_ref_anae_list[1]<-(temp_ref_anae[Ite]+(as.numeric(temp_ref_anae_list)))
l[9]=paste(temp_ref_anae_list,collapse=",")
#################################################################################
photo=round(runif(Nsims, min=-5, max=5),0)
photo_list <- unlist(strsplit(l[10],","))
photo_list[1]<-(photo[Ite]+(as.numeric(photo_list)))
l[10]=paste(photo_list,collapse=",")
################
RFLAT=round(runif(Nsims, min=-5, max=5),0)
RFLAT_list <- unlist(strsplit(l[11],","))
RFLAT_list[1]<-(RFLAT[Ite]+(as.numeric(RFLAT_list)))
l[11]=paste(RFLAT_list,collapse=",")
################################################################################
hydro=round(runif(Nsims, min=-15, max=15),0)
hydro_list <- unlist(strsplit(l[12],","))
hydro_list[1]<-(hydro[Ite]+(as.numeric(hydro_list)))
l[12]=paste(hydro_list,collapse=",")
#############################################################################
SOL=round(runif(Nsims, min=-0.5, max=1),2)
SOL_list <- unlist(strsplit(l[18],","))
SOL_list[1]<-(SOL[Ite]+(as.numeric(SOL_list)))

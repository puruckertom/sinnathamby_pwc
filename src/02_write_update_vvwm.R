...
######################################################################################
anae_aq=round(runif(Nsims, min=0.95, max=1.05),2)
...
#####################################################################################
temp_ref_anae=round(runif(Nsims, min=0.95, max=1.05),2)
...
###########################################################################
FROC2=round(runif(Nsims, min=0.90, max=1.10),2)#Fraction of organic carbon on sediment in benthic region.
...
l[45]=paste(DOC2_a)
###########################################################################
BNMAS=round(runif(Nsims, min=0.90, max=1.10),3)#Areal concentration of biosolids in benthic region (g/m2)
BNMAS_a<-round((BNMAS[Ite]*(as.numeric(l[46]))),3)
l[46]=paste(BNMAS_a)
###########################################################################
SUSED=round(runif(Nsims, min=0.90, max=1.10),2)#Suspended solids concentration in water column (mg/L)
SUSED_a<-round((SUSED[Ite]*(as.numeric(l[48]))),2)
l[48]=paste(SUSED_a)
###########################################################################
CHL=round(runif(Nsims, min=0.90, max=1.10),2)#Chlorophyll concentration in water column (mg/L)
CHL_a<-round((CHL[Ite]*(as.numeric(l[49]))),3)
l[49]=paste(CHL_a)
###########################################################################
FROC1=round(runif(Nsims, min=0.95, max=1.05),2)#Fraction of organic carbon on suspended sediment in water column.
FROC1_a<-round((FROC1[Ite]*(as.numeric(l[50]))),3)
l[50]=paste(FROC1_a)
###########################################################################
DOC1=round(runif(Nsims, min=0.95, max=1.05),2)#Concentration of dissolved organic carbon in water column (mg/L)
DOC1_a<-round((DOC1[Ite]*(as.numeric(l[51]))),1)
l[51]=paste(DOC1_a)
###########################################################################
PLMAS=round(runif(Nsims, min=0.90, max=1.10),2)#Concentration of biosolids in water column (mg/L)

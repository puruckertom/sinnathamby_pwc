...
uslep=round(runif(Nsims, min=0.1, max=1),2);inputdf <- cbind(inputdf, uslep)#uslep0.10 (extensive practices) to 1.0 (no supporting practices)
slp=round(runif(Nsims, min=0.001, max=5),3);inputdf <- cbind(inputdf, slp)#land slope (%).
hl=round(runif(Nsims, min=1, max=5000),1);inputdf <- cbind(inputdf, hl)#hydraulic length (m)
...
MNGN=round(runif(Nsims, min=0.006, max=0.47),3);inputdf <- cbind(inputdf, MNGN)#manning's n
depth=round(runif(Nsims, min=1, max=140),1);inputdf <- cbind(inputdf, depth)#maximum soil depth is 139 cm. Root depth can't deeper than the depth of the soil layer
...
holdup=round(runif(Nsims, min=0.1, max=0.3),2);inputdf <- cbind(inputdf, holdup)#canopy interception
...
fc=round(runif(Nsims, min=0.25, max=0.43),2);inputdf <- cbind(inputdf, fc) #fc
WP=round(runif(Nsims, min=0.05, max=0.24),2);inputdf <- cbind(inputdf, WP) #wp
OC=round(runif(Nsims, min=0.01, max=4),2);inputdf <- cbind(inputdf, OC) #oc
...
app_eff=round(runif(Nsims, min=0.1, max=0.99),2);inputdf <- cbind(inputdf, app_eff)#application rate
...
SUSED=round(runif(Nsims, min=0.005, max=80),3);inputdf <- cbind(inputdf, SUSED)#Suspended solids concentration in water column (mg/L)
...
PLMAS=round(runif(Nsims, min=0.001, max=10),3);inputdf <- cbind(inputdf, PLMAS)#Concentration of biosolids in water column (mg/L)
...
bf=round(runif(Nsims, min=0.0, max=10),2);inputdf <- cbind(inputdf, bf)#Provids an additional constant flow through the waterbody m3/s

...
#############################################################################################################
Ite <- 1
df <- read.table(paste(newdir,"/","output",".zts", sep=""), header= TRUE, sep= "",
...
timearray<-seq(as.Date("1961-01-01"), as.Date("2014-12-31"), by="days")
...
#write.csv(outputdf, file = paste(pwcdir, "io/test.csv", sep = ""))
rm(outputdf)
